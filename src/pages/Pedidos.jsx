...
import { ShoppingBag, Plus, X, ChevronDown, DollarSign } from 'lucide-react'
...
export default function Pedidos() {
  ...
  const [formEncargo, setFormEncargo] = useState({ clienteNombre: '', clienteTelefono: '', descripcion: '', talla: '', color: '', totalVenta: '', abonoInicial: '', notas: '' })
  const [formTemu, setFormTemu] = useState({ descripcion: '', categoria: '', talla: '', color: '', precioEstimado: '', linkTemu: '', notas: '' })
  ...
  const crearEncargo = async (e) => {
    e.preventDefault()
    if (!formEncargo.clienteNombre || !formEncargo.descripcion || !formEncargo.totalVenta) { toast.error('Nombre, descripción y total son obligatorios'); return }
    setGuardando(true)
    try {
      ...
      setFormEncargo({ clienteNombre: '', clienteTelefono: '', descripcion: '', talla: '', color: '', totalVenta: '', abonoInicial: '', notas: '' })
      cargarEncargos()
  ...
  const crearTemu = async (e) => {
    ...
    try {
      await api.post('/api/pedidos', {
        descripcion: formTemu.descripcion,
      ...
      setFormTemu({ descripcion: '', categoria: '', talla: '', color: '', precioEstimado: '', linkTemu: '', notas: '' })
      cargarTemu()
    } catch (err) { toast.error(err.response?.data?.error || 'Error guardando') }
    finally { setGuardando(false) }
  }
                ...
                <button key={e._id} onClick={() => setModalPedido({ ...e, tipo: 'encargo' })}
                  ...
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-purple-50 flex items-center justify-center flex-shrink-0">
                      <span className="font-black text-[#7C3AED] text-xs">{getIniciales(e.cliente?.nombre || '')}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className="font-bold text-gray-900 text-sm truncate">{e.cliente?.nombre}</p>
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full border flex-shrink-0 ${colorEstadoCliente(e.estado)}`}>{labelEstadoCliente(e.estado)}</span>
                      </div>
                      <p className="text-sm text-gray-400 mt-0.5 truncate">{e.descripcion}</p>
                      <div className="mt-2">
                        <div className="flex justify-between text-xs text-gray-400 mb-1">
                          <span>{formatCOP(e.totalPagado)} pagado</span>
            ...
            {pedidosTemu.map(p => (
              <button key={p._id} onClick={() => setModalPedido({ ...p, tipo: 'temu' })}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 text-sm leading-tight truncate">{p.descripcion}</p>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {p.categoria && <span className="text-xs text-gray-400 capitalize">{p.categoria}</span>}
                      {p.talla && <span className="text-xs text-gray-400">Talla {p.talla}</span>}
                      {p.precioEstimado && <span className="text-xs font-bold text-gray-700">{formatCOP(p.precioEstimado)}</span>}
                    </div>
                    <p className="text-xs text-gray-400 mt-1">{fechaRelativa(p.createdAt)}</p>
                  </div>
                  <span className={`text-xs font-bold px-2 py-1 rounded-full border flex-shrink-0 ${colorEstadoTemu(p.estado)}`}>{labelEstadoTemu(p.estado)}</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/40">
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-xl max-h-[85vh] overflow-y-auto">
            <div className="p-5">
              ...
              <p className="text-base text-gray-700 mb-4">{modalPedido.descripcion}</p>

              <div className="mb-4">
                <label className="block text-sm font-bold text-gray-700 mb-2">Estado actual</label>
                <div className="relative">
                  <select value={modalPedido.estado}
                    onChange={e => actualizarEstado(modalPedido._id, e.target.value, modalPedido.tipo)}
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-base focus:outline-none focus:ring-2 focus:ring-purple-400 appearance-none">
                    {(modalPedido.tipo === 'encargo' ? ESTADOS_CLIENTE : ESTADOS_TEMU).map(est => (
                      <option key={est} value={est}>{modalPedido.tipo === 'encargo' ? labelEstadoCliente(est) : labelEstadoTemu(est)}</option>
                    ))}
                  </select>
              ...
              {modalPedido.tipo === 'encargo' && (
                <div className="bg-purple-50 rounded-xl p-4 mb-4">
                  <div className="flex justify-between mb-1"><span className="text-sm text-gray-500">Total</span><span className="font-bold">{formatCOP(modalPedido.totalVenta)}</span></div>
                  <div className="flex justify-between mb-1"><span className="text-sm text-gray-500">Pagado</span><span className="font-bold text-emerald-600">{formatCOP(modalPedido.totalPagado)}</span></div>
                  <div className="flex justify-between"><span className="text-sm font-bold">Falta</span><span className="font-black text-red-600">{formatCOP(modalPedido.saldoPendiente)}</span></div>
                </div>
              )}
              ...
              {modalPedido.tipo === 'encargo' && modalPedido.saldoPendiente > 0 && (
                !mostrarAbono ? (
                  <button onClick={() => setMostrarAbono(true)}
                    className="w-full flex items-center justify-center gap-2 bg-[#7C3AED] text-white font-bold py-3 rounded-xl hover:bg-[#5B21B6] mb-3">
                  ...
                  <div className="space-y-3 mb-3">
                    <input type="number" min="1" value={montoAbono} onChange={e => setMontoAbono(e.target.value)}
                      placeholder="Monto del abono ($)" autoFocus
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setMostrarAbono(false)} className="flex-1 py-3 rounded-xl border border-gray-200 font-bold">Cancelar</button>
                      <button onClick={registrarAbono} disabled={guardando} className="flex-1 bg-[#7C3AED] text-white font-bold py-3 rounded-xl disabled:opacity-60">
            ...
            <div className="p-5">
              ...
              <form onSubmit={crearEncargo} className="space-y-4">
                {[
                  ...
                  { key: 'descripcion', label: '¿Qué encargó? *', placeholder: 'Vestido largo azul talla M' },
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1.5">Anticipo ($)</label>
          ...
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="p-5">
              ...
              <form onSubmit={crearTemu} className="space-y-4">
                {[
                  { key: 'descripcion', label: '¿Qué vas a pedir? *', placeholder: '10 blusas rosadas talla M' },
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Precio estimado ($)</label>
