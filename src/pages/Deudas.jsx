...
import { Users, Plus, X, DollarSign, ChevronRight, Search, Edit2, Trash2, Minus } from 'lucide-react'
...
export default function Deudas() {
  ...
  const agregarAlCarrito = (p) => {
    ...
    setItemsCarrito(prev => {
      ...
      return [...prev, { productoId: p._id, nombreProducto: p.nombre, cantidad: 1, maxCantidad: p.cantidad, precioVenta: p.precioVenta, precioCompra: p.precioCompra, talla: p.talla, color: p.color }]
    })
  }
  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-28 bg-white rounded-2xl border border-purple-100 animate-pulse" />)}</div>
      ) : deudoras.length === 0 ? (
              ...
              <button key={d._id} onClick={() => abrirModal(d)}
                ...
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-full bg-purple-50 flex items-center justify-center flex-shrink-0">
                    <span className="font-black text-[#7C3AED] text-base">{getIniciales(d.cliente?.nombre || '')}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-black text-gray-900 text-lg leading-tight truncate">{d.cliente?.nombre}</p>
                      <span className={`text-sm font-bold px-2.5 py-1 rounded-full border flex-shrink-0 ${colorEstadoCredito(d.estado)}`}>
                    ...
                    {d.items?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-1.5">
                        {d.items.map((item, i) => (
                          <span key={i} className="text-sm font-semibold text-gray-700 bg-gray-100 px-2 py-0.5 rounded-lg">
                            {item.cantidad > 1 ? `${item.cantidad}x ` : ''}{item.nombreProducto}
                          </span>
                        ))}
                      </div>
                    )}
                    <div className="mt-2">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-gray-500 font-semibold">Pagado: <span className="text-gray-700">{formatCOP(d.totalPagado)}</span></span>
                        <span className="font-bold text-red-600">Falta: {formatCOP(d.saldoPendiente)}</span>
                      </div>
                    ...
                    <p className="text-sm text-gray-400 mt-1.5">{fechaRelativa(d.createdAt)}</p>
                  </div>
                  <div className="flex items-start gap-2 flex-shrink-0">
                    <button onClick={e => { e.stopPropagation(); setEditandoDeuda(d); setFormEdit({ notas: d.notas || '' }) }}
                      className="p-2.5 rounded-xl hover:bg-purple-50 text-gray-400 hover:text-[#7C3AED] transition-colors">
                    ...
                    <ChevronRight className="w-5 h-5 text-gray-300 mt-2.5" />
                  </div>
                </div>
              </button>
            )
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/40">
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-xl max-h-[85vh] overflow-y-auto">
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-purple-50 flex items-center justify-center">
                    <span className="font-black text-[#7C3AED] text-base">{getIniciales(modalDeudora.cliente?.nombre || '')}</span>
                  </div>
              ...
              {/* Items de la deuda — más grandes */}
              {modalDeudora.items?.length > 0 && (
                <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-2">
                  {modalDeudora.items.map((item, i) => (
                    <div key={i} className="flex justify-between items-center">
                      <span className="text-base font-semibold text-gray-800">{item.cantidad > 1 ? `${item.cantidad}x ` : ''}{item.nombreProducto}</span>
                      <span className="font-black text-gray-900 text-base">{formatCOP(item.precioUnitario * item.cantidad)}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="bg-purple-50 rounded-xl p-4 mb-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-gray-900 text-base">{formatCOP(modalDeudora.totalVenta)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-base text-gray-600">Pagado</span>
                  <span className="font-bold text-emerald-600 text-base">{formatCOP(modalDeudora.totalPagado)}</span>
                </div>
                <div className="flex justify-between items-center border-t border-purple-200 pt-2 mt-1">
                  <span className="text-base font-bold text-gray-700">Falta pagar</span>
                  <span className="font-black text-red-600 text-xl">{formatCOP(modalDeudora.saldoPendiente)}</span>
                </div>
              </div>

              <h3 className="font-bold text-gray-900 text-base mb-3">Historial de abonos</h3>
              {modalDeudora.abonos?.length > 0 ? (
                <div className="space-y-2 mb-4">
                  {modalDeudora.abonos.map((a, i) => (
                    <div key={i} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                      <div>
                        <p className="font-bold text-gray-900 text-base">{formatCOP(a.monto)}</p>
                        {a.nota && <p className="text-sm text-gray-500">{a.nota}</p>}
                      </div>
                      <p className="text-sm text-gray-400">{fechaRelativa(a.fecha)}</p>
                    </div>
                  ))}
                </div>
              ) : <p className="text-base text-gray-400 mb-4">Sin abonos registrados</p>}

              {modalDeudora.estado !== 'pagado' && (
                !mostrarFormAbono ? (
                  <button onClick={() => setMostrarFormAbono(true)}
                    className="w-full flex items-center justify-center gap-2 bg-[#7C3AED] text-white font-bold py-4 rounded-xl hover:bg-[#5B21B6] transition-colors text-base">
                    <DollarSign className="w-5 h-5" /> Registrar abono
                  </button>
                ) : (
                  <form onSubmit={handleAbono} className="space-y-3">
                    <div>
                      <label className="block text-base font-bold text-gray-700 mb-1.5">Monto del abono ($)</label>
                    ...
                    <div className="flex gap-3">
                      <button type="button" onClick={() => setMostrarFormAbono(false)}
                        className="flex-1 py-3 rounded-xl border border-gray-200 font-bold text-gray-700 hover:bg-gray-50 text-base">Cancelar</button>
                      <button type="submit" disabled={guardando}
                        className="flex-1 bg-[#7C3AED] text-white font-bold py-3 rounded-xl hover:bg-[#5B21B6] disabled:opacity-60 text-base">
          ...
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="p-5">
              ...
              <form onSubmit={handleNuevaDeuda} className="space-y-4">
                ...
                <div>
                  <label className="block text-base font-bold text-gray-700 mb-1.5">Nombre de la clienta *</label>
                ...
                {itemsCarrito.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-base font-bold text-gray-600">Productos</p>
                    ...
                    <div className="flex justify-between items-center bg-red-50 rounded-xl p-3">
                      <span className="font-bold text-gray-700 text-base">Total que debe</span>
                      <span className="font-black text-red-600 text-2xl">{formatCOP(totalDeuda)}</span>
                    </div>
                  </div>
                )}
                ...
                {itemsCarrito.length > 0 && (
                  <div>
                    <label className="block text-base font-bold text-gray-700 mb-1.5">Abono inicial (opcional)</label>
                    ...
                    {abonoInicial && parseFloat(abonoInicial) > 0 && (
                      <div className="mt-2 space-y-1">
                        <p className="text-sm text-gray-500">Queda debiendo: <span className="font-bold text-red-500">{formatCOP(saldoTrasAbono)}</span></p>
                        {productosCubiertos().length > 0 && (
                          <div className="bg-emerald-50 rounded-xl p-3">
                            <p className="text-sm font-bold text-emerald-700 mb-1">✓ Quedarían pagados y pasarían a ventas:</p>
                            {productosCubiertos().map((item, i) => (
                              <p key={i} className="text-sm text-emerald-600">{item.unidadesCubiertas}x {item.nombreProducto}</p>
                            ))}
                          </div>
                        )}
                      </div>
          ...
          <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-xl">
            <div className="p-5">
              ...
              <div className="bg-purple-50 rounded-xl p-3 mb-4">
                <p className="font-bold text-gray-900 text-base">{editandoDeuda.cliente?.nombre}</p>
                <p className="text-sm text-gray-600 mt-1">{editandoDeuda.items?.map(i => `${i.cantidad > 1 ? i.cantidad + 'x ' : ''}${i.nombreProducto}`).join(', ')}</p>
              </div>
          ...
          <div className="bg-white w-full md:max-w-sm rounded-t-3xl md:rounded-2xl shadow-xl">
            <div className="p-5">
              ...
              <div className="bg-red-50 rounded-xl p-4 mb-5">
                <p className="font-bold text-gray-900 text-base">{confirmando.cliente?.nombre}</p>
                <p className="text-sm text-gray-600 mt-1">Falta: {formatCOP(confirmando.saldoPendiente)} — Esta acción no devuelve el stock.</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
